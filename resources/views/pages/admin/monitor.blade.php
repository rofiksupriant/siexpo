        ...
        <div class="card">
            ...
            <table id="" class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$monitor->name}}</td>
                        <td>{{$monitor->price}}</td>
                        <td>{{$monitor->brand?$monitor->brand->name:"-"}}</td>
                        <td>
                            <div class="dropdown dropleft">
                                ...
                                <div x-placement="bottom-end" class="dropdown-menu" >
                                    <a id="updateAction" data-id={{$monitor->id}} href="#" class="dropdown-item" type="button" style="opacity: 0.5;"><i class="fas fa-edit"> Edit</i></a>
                                    <a id="deleteAction" data-id={{$monitor->id}} href="#" class="dropdown-item" type="button" style="opacity: 0.5;"><i class="fas fa-trash"> Delete</i></a>
                                </div>
                            </div>
                        </td>
                    </tr>
            ...
            <div class="d-flex">
                <div class="mx-auto mt-5">
                    {{ $monitors->links('pagination::bootstrap-4') }}
                </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                ...
                <form id="create" action="{{ route('create_monitor') }}" method="POST">
                    @csrf
...
@push('script')
    <script>
        ...
        id = $(this).data('id');
        var routeUpdate = "{{ route('update_monitor',":id") }}";
        let url = "{{ route('form_update_monitor',":id") }}";
        ...
        $.ajax({
            ...
            success: function (monitor) {
                ...
                $(".modal-body #name").val(monitor.name);
                $(".modal-body #price").val(monitor.price);
                $(".modal-body #brand").val(monitor.brand_id);
    ...
    $(document).on("click", "#deleteAction", function () {
        id = $(this).data('id');
        var routeDelete = "{{ route('delete_monitor',":id") }}";
