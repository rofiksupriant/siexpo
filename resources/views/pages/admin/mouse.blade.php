        ...
        <div class="card">
            ...
            <table id="" class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$mouse->name}}</td>
                        <td>{{$mouse->price}}</td>
                        <td>{{$mouse->brand?$mouse->brand->name:"-"}}</td>
                        <td>
                            <div class="dropdown dropleft">
                                ...
                                <div x-placement="bottom-end" class="dropdown-menu" >
                                    <a id="updateAction" data-id={{$mouse->id}} href="#" class="dropdown-item" type="button" style="opacity: 0.5;"><i class="fas fa-edit"> Edit</i></a>
                                    <a id="deleteAction" data-id={{$mouse->id}} href="#" class="dropdown-item" type="button" style="opacity: 0.5;"><i class="fas fa-trash"> Delete</i></a>
                                </div>
                            </div>
                        </td>
                    </tr>
            ...
            <div class="d-flex">
                <div class="mx-auto mt-5">
                    {{ $mouses->links('pagination::bootstrap-4') }}
                </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                ...
                <form id="create" action="{{ route('create_mouse') }}" method="POST">
                    @csrf
...
@push('script')
    <script>
        ...
        id = $(this).data('id');
        var routeUpdate = "{{ route('update_mouse',":id") }}";
        let url = "{{ route('form_update_mouse',":id") }}";
        ...
        $.ajax({
            ...
            //     _token: '{{ csrf_token() }}'
            // },
            success: function (mouse) {
                console.log(routeUpdate);
                $(".modal-body #name").val(mouse.name);
                $(".modal-body #price").val(mouse.price);
                $(".modal-body #brand").val(mouse.brand_id);
    ...
    $(document).on("click", "#deleteAction", function () {
        id = $(this).data('id');
        var routeDelete = "{{ route('delete_mouse',":id") }}";
