        ...
        <div class="card">
            ...
            <table id="" class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$vga->name}}</td>
                        <td>{{$vga->price}}</td>
                        <td>{{$vga->brand?$vga->brand->name:"-"}}</td>
                        <td>
                            <div class="dropdown dropleft">
                                ...
                                <div x-placement="bottom-end" class="dropdown-menu" >
                                    <a id="updateAction" data-id={{$vga->id}} href="#" class="dropdown-item" type="button" style="opacity: 0.5;"><i class="fas fa-edit"> Edit</i></a>
                                    <a id="deleteAction" data-id={{$vga->id}} href="#" class="dropdown-item" type="button" style="opacity: 0.5;"><i class="fas fa-trash"> Delete</i></a>
                                </div>
                            </div>
                        </td>
                    </tr>
            ...
            <div class="d-flex">
                <div class="mx-auto mt-5">
                    {{ $vgas->links('pagination::bootstrap-4') }}
                </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                ...
                <form id="create" action="{{ route('create_vga') }}" method="POST">
                    @csrf
...
@push('script')
    <script>
        ...
        id = $(this).data('id');
        var routeUpdate = "{{ route('update_vga',":id") }}";
        let url = "{{ route('form_update_vga',":id") }}";
        ...
        $.ajax({
            ...
            //     _token: '{{ csrf_token() }}'
            // },
            success: function (vga) {
                console.log(routeUpdate);
                $("#update #name").val(vga.name);
                $("#update #price").val(vga.price);
                $("#update #brand").val(vga.brand_id);
    ...
    $(document).on("click", "#deleteAction", function () {
        id = $(this).data('id');
        var routeDelete = "{{ route('delete_vga',":id") }}";
