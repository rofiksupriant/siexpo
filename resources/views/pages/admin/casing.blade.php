        ...
        <div class="card">
            ...
            <table id="" class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$casing->name}}</td>
                        <td>{{$casing->price}}</td>
                        <td>{{$casing->brand?$casing->brand->name:"-"}}</td>
                        <td>
                            <div class="dropdown dropleft">
                                ...
                                <div x-placement="bottom-end" class="dropdown-menu" >
                                    <a id="updateAction" data-id={{$casing->id}} href="#" class="dropdown-item" type="button" style="opacity: 0.5;"><i class="fas fa-edit"> Edit</i></a>
                                    <a id="deleteAction" data-id={{$casing->id}} href="#" class="dropdown-item" type="button" style="opacity: 0.5;"><i class="fas fa-trash"> Delete</i></a>
                                </div>
                            </div>
                        </td>
                    </tr>
            ...
            <div class="d-flex">
                <div class="mx-auto mt-5">
                    {{ $casings->links('pagination::bootstrap-4') }}
                </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-body">
                ...
                <form id="create" action="{{ route('create_casing') }}" method="POST">
                    @csrf
...
@push('script')
    <script>
        ...
        id = $(this).data('id');
        var routeUpdate = "{{ route('update_casing',":id") }}";
        let url = "{{ route('form_update_casing',":id") }}";
        ...
        $.ajax({
            ...
            //     _token: '{{ csrf_token() }}'
            // },
            success: function (casing) {
                console.log(routeUpdate);
                $("#update #name").val(casing.name);
                $("#update #price").val(casing.price);
                $("#update #brand").val(casing.brand_id);
    ...
    $(document).on("click", "#deleteAction", function () {
        id = $(this).data('id');
        var routeDelete = "{{ route('delete_casing',":id") }}";
